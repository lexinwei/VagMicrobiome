...
openxlsx::write.xlsx(l, paste0('./data/instrain_gene/MTMG_gene_vs_genome.adj.xlsx'))
# volcano_plot(statDF3, x = 'pi_logFC', y = 'pi_pvalue.adj', FC_cutoff = 1.2)
save(statDF3_all, file = './data/RData/MTMG_statDF3_all.RData')
save(l, file = './data/RData/MTMG_ll.RData')
###### scatter plot for each species, SNVs vs SNS, dN/dS vs pNpS ######
load('./data/RData/MTMG_statDF3_all.RData')
cairo_pdf(file = './plot/MTMG_scatterplot_dNdS_vs_pNpS.pdf', width = 4.47, height = 4.3, onefile = T)
...
if(T){
  ...
  cairo_pdf('./plot/MTMG_distribution_of_dnds_pnps_genes.pdf', width = 3.94, height = 4.39)
...
# 1. Create the histogram plot
  ...
  # 3. Align the two plots and then overlay them.
  aligned_plots <- cowplot::align_plots(phist, pdensity, align="hv", axis="tblr")
  cairo_pdf('./plot/MTMG_stat_of_dnds_pnps_genes_include_0.pdf', width = 3.94, height = 4.39)
  cowplot::ggdraw(aligned_plots[[1]]) + cowplot::draw_plot(aligned_plots[[2]])
  dev.off()
}
...
if(T){
  ...
  cairo_pdf('./plot/MTMG_pi_high_low_gene_circle_packing_2.pdf', width = 11.97 , height = 8.35)
  print(cP)
  dev.off()
}
...
cP = ggraph(mygraph, layout = 'circlepack') +  # weight = size
  ...
  theme_void() + theme(legend.text = element_text(size = 13), legend.key.size = unit(0.5, 'cm'), 
                       legend.title = element_text(size = 13), plot.margin = unit(c(0,1,0,0), 'cm'))
cairo_pdf('./plot/MTMG_pNpS_gene_circle_packing_3.pdf', width = 8.81 , height = 4.76)
print(cP)
dev.off()
...
cP = ggraph(mygraph, layout = 'circlepack') +  # weight = size
  ...
  theme_void() + theme(legend.text = element_text(size = 13), legend.key.size = unit(0.5, 'cm'), 
                       legend.title = element_text(size = 13), plot.margin = unit(c(0,1,0,0), 'cm'))
cairo_pdf('./plot/MTMG_dNdS_gene_circle_packing_3.pdf', width = 6.92 , height = 4.36)
print(cP)
dev.off()
...
#   geom_point()
cairo_pdf('./plot/MTMG_key_genes_evolutionary_metrics.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
...
# ggplot(statKey, aes(x = meanGen_pNpS, y = meanGen_pi)) +
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics_MTMG_new.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
...
cairo_pdf(file =  './plot/MTMG_gene_for_each_species_evolutionary_metrics.pdf', width = 11.14, height = 7.14, onefile = T)
for(sp in unique(statDF3_all$Species)){
...
cairo_pdf('./plot/MTMG_dnds_freq_barplot.pdf', width = 5.21, height = 5.60, onefile = T)
print(dP)
print(dPs2)
print(dPs3)
dev.off()
...
cairo_pdf('./plot/MTMG_dnds_bigger1_barplot.pdf', width = 5.21, height = 5.60, onefile = T)
print(dP)
print(dP2)
print(dP3)
dev.off()
