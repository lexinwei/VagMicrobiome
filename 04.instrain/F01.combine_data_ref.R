...
if(T){
  ...
  for(ss in swabData$fqID){
    print(ss)
    geneDat = read.delim2(paste0('./instrain_res/to_NCBI_ref2/gene_info/', ss, '_bwa.IS_gene_info.tsv'))
    dt = fread(paste0('./instrain_res/to_NCBI_ref2/genes_SNP_count/', ss, '_bwa_genes_SNP_count.csv.gz'), 
...
save(refDF, file = './data/RData/refDF_minCovBreath10_re.RData')
...
if(plotFlag){
  pdf('./plot/ref_genome_info_with_dn_ds_new_RE.pdf', width = 15, height = 8)
  labDF = data.frame(
