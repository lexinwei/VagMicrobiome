...
# magsAbu = read.csv('./data/mags_abundance.csv')
# magsAbu = melt(magsAbu)
magDF0 = magDF
magDF0$coverage %<>% as.numeric()
magDF = data.frame()
for(samp in unique(magDF0$sample)){
  subf = magDF0[magDF0$sample == samp, ]
  subf$abu = subf$coverage/sum(subf$coverage)
  magDF = rbind(magDF, subf)
}

load('./data/spName.RData')
load('./data/metaphlanDF.RData')
# CN <- combn(na.omit(colnames(metaData[, -1])), 2, simplify = FALSE)
# corV = data.frame()
# cor(metaData, use = 'pair')
# for(i in 1:length(CN)){
#   df = metaData[, CN[[i]]]
#   cor = glm(as.formula(str_c(CN[[i]], collapse = '~')), metaData)
#   vv = c(CN[[i]], cor[1,2])
#   corV = rbind(corV, vv)
# }
...
# refMetData$if_lac = 'No'
# refMetData$if_lac[refMetData$genus == 'Lactobacillus'] = 'Yes'
# table(refMetData$if_lac)
...
# refMetData_sub = refMetData[, c('nucl_diversity', 'coverage', 'breadth', 
#                             'breadth_minCov', 'dn_ds_common')]
# pdf('./plot/corr_of_diversity_vs_coverage.pdf', height = 7.52, width = 7.83)
...
# allMetData$abu = metaphlanDF$relative_abundance[match(
#                                                       paste0(allMetData$pt_ID.u, '|', allMetData$species),
...
if(T){
  ...
  xs = c('abu','trimester', 'Age_char', 'BMI_char', 'Ethnicity', 'Language', 'Marriage', 'FOB', 'Employed', 'Housing', 
  ...
  if(plotFlag){
    xlsx::write.xlsx(comp_DF, file = './Routput/genome_wise_nucl_div_dnds_metadata_pairwise_comp_2_add_abu_new.xlsx')  
    xlsx::write.xlsx(uniRes, file = './Routput/genome_wise_nucl_div_dnds_metadata_uni_model_2_add_abu_new.xlsx')
  }
}
# comp_DF = xlsx::read.xlsx('./Routput/genome_wise_nucl_div_dnds_metadata_pairwise_comp_2.xlsx', sheetIndex = 1)

##### lme regression fixed model #####
uniRes = xlsx::read.xlsx('./Routput/genome_wise_nucl_div_dnds_metadata_uni_model_2_add_abu_new.xlsx', sheetIndex = 1)
load('./data/metaphlanDF.RData')
...
xlsx::write.xlsx(res_mulisig, './Routput/nucl_div_dnds_lme_regression_fixed_add_abu_new.xlsx')


##### plot boxplot for candidate species and variable all references and mags #####
if(plotFlag){
  pdf('./plot/genome_wise_nucl_div_dnds_metadata_all_3.pdf', width = 10, height = 7)
}
x = 'Ethnicity'
...
if(plotFlag){
  pdf('./plot/genome_wise_nucl_div_dnds_metadata_super_sig_2.pdf', width = 10, height = 7)
}
...
res_filter = res[res$y %in% c('nucl_diversity', 'dn_ds_common') &
                   res$x %in% xs[xs != 'abu'], ] # dn_ds_common, pnps, dnds
nrow(res_filter[grep('NCBI', res_filter$species), ])
...
uniqueVar$y %<>% str_replace_all('_common', '')
...
col_fun$col = '#ECF0D9'
col_fun$col[uniqueVar$lme.multi.P_value < 0.05] = '#FFA18F' 
table(col_fun$col)
library(circlize)
pdf('./plot/circos_association_genitic_diversity_2_new.pdf', width = 14, height = 15)
{chordDiagram(uniqueVar[, c('x', 'sp_y')], order = ord, annotationTrack = "grid", 
             grid.col = grid.col, col = col_fun$col, transparency = 0.3,
...
pdf('./plot/circos_association_genitic_diversity_ncbi_new.pdf', width = 9, height = 10)
nrow(uniqueVar[grep('NCBI', uniqueVar$species), c('x', 'sp_y')])
nrow(uniqueVar[which(grepl('NCBI', uniqueVar$species) & uniqueVar$lme.multi.P_value<0.05), c('x', 'sp_y')])
{chordDiagram(uniqueVar[grep('NCBI', uniqueVar$species), c('x', 'sp_y')], order = ord, annotationTrack = "grid", 
             grid.col = grid.col, col = col_fun$col[grep('NCBI', uniqueVar$species)], transparency = 0.3,
             annotationTrackHeight = mm_h(1.5),
...
{chordDiagram(uniqueVar[grep('\\.fa', uniqueVar$species), c('x', 'sp_y')], order = ord, annotationTrack = "grid", 
              grid.col = grid.col, col = col_fun$col[grep('\\.fa', uniqueVar$species)], transparency = 0.3,
              annotationTrackHeight = mm_h(1.5),
              preAllocateTracks = list(track.height = 0.5))
  circos.track(track.index = 1,
               panel.fun = function(x, y) {
