...
##### ref sample-wise profile #####
ided_magsDF = data.frame()
for(ss in swabData$fqID){
  tmpDat = read.delim2(paste0('./instrain_res/ref_profile/genome_info/', ss, '_bwa.IS_genome_info.tsv'))
  tmpDat = tmpDat[, c("genome", "coverage", "breadth", "nucl_diversity", "length",
...
if(F){
  pdf('./plot/ref_genome_wise_nucl_div.pdf', width = 12, height = 4)
  for (sp in unique(ided_magsDF$genome)) {
    ...
    p = ggplot(tmpDF, aes(x = pt_ID.u, y = nucl_diversity, color = pt_ID)) +
      ...
      labs(title = sp, y = paste0('Nucleotide diversity'), x = '')
    print(p)
  }
  dev.off()
}
...
if(T){
  ...
  vars = c('Age_char', 'BMI_char', 'Ethnicity', 'Language', 'Marriage', 'FOB', 'Employed', 'Housing', 
...
x = merge(lmeResMerge, aovRes, by = c('species', 'var'), all = T)
...
dim(aovRes)
table(!is.na(x$lm.uni.P_value))
uniVarSigDF = x[x$lm.uni.P_value <= 0.05, ]
uniVarSigDF$Var = str_replace_all(uniVarSigDF$var, '_char', '')
uniVarSigDF$lm.uni.P_value_log = -log10(uniVarSigDF$lm.uni.P_value)
uniVarSigDF$lm.multi.P_value_log = -log10(uniVarSigDF$lme.multi.P_value)
uniqueVar = uniVarSigDF[!duplicated(uniVarSigDF[, c('species', 'Var')]), c('species', 'Var', 'lm.uni.P_value_log', 'lme.multi.P_value')]
ord = c(sort(table(uniqueVar$Var)) %>% names(),
        sort(table(uniqueVar$species)) %>% names())
grid.col = c(rep('#80B1D3', length(table(uniqueVar$species))), rep('#E5C494', length(table(uniqueVar$Var))))
names(grid.col) = c(table(uniqueVar$species) %>% names(), table(uniqueVar$Var) %>% names())
col_fun = data.frame(
  uniqueVar$species,
  uniqueVar$Var
)
...
pdf('./plot/circos_association_nucl_diversity.pdf', width = 9, height = 12)
chordDiagram(uniqueVar[, 1:3], order = ord, annotationTrack = "grid", 
