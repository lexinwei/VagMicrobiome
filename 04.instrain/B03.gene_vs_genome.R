...
load('./data/RData/MAGs_ll.RData')
load('./data/RData/colorList.RData')
load('./data/RData/MAGs_statDF3_all.RData')
bin2rank$lastTaxa = bin2rank$species
bin2rank$lastTaxa[bin2rank$species == ""] = bin2rank$genus[bin2rank$species == ""]
bin2rank$Species = paste0(bin2rank$lastTaxa, ' | ', bin2rank$ID)
bin2rank2 = bin2rank[paste0(bin2rank$ID, '.fa') %in% srgDF$genome, ]
a = read.table('./mags_for_gene.txt')
b = bin2rank[bin2rank$ID %in% a$V1, ]
setdiff(a$V1, b$ID)
if(F){ # parse hmmscan(pfam) tblout results
  ref_gene_anno = read.delim2('./instrain_res/to_MAGs_139_2/gene/mags_139_genomes2_tblout.txt', sep = '\t', header = F)
  target_name = c()
  accession = c()
  query_name = c()
  E_value = c()
  ...
  range(ref_tbl_table$E_value, na.rm = T)
  ...
  table(ref_tbl_table2$target_name %in% c('MucBP', 'Muc_B2', 'MucBP_2'))
  ref_tbl_table2.2 = ref_tbl_table2[which(ref_tbl_table2$E_value < 1e5), ]
  table(ref_tbl_table2.2$target_name %in% c('MucBP', 'Muc_B2', 'MucBP_2'))
  save(ref_tbl_table2.2, file = './data/RData/ref_tbl_table2.2.RData') # more gene were annotated as mucin but not the top annotation
  ref_tbl_table3 = ref_tbl_table2[which(!duplicated(ref_tbl_table2$query_name)), ]
  table(ref_tbl_table3$target_name %in% c('MucBP', 'Muc_B2', 'MucBP_2'))
  save(ref_tbl_table3, file = './data/RData/ref_tbl_table3.RData')
}
load('./data/RData/ref_tbl_table2.2.RData')
table(ref_tbl_table2.2$target_name %in% c('MucBP', 'Muc_B2', 'MucBP_2','Gram_pos_anchor'))
ref_tbl_table2.22 = ref_tbl_table2.2$query_name[ref_tbl_table2.2$target_name %in% 
                                                  c('MucBP', 'Muc_B2', 'MucBP_2','Gram_pos_anchor')]
baseDir = './instrain_res/to_MAGs_139_2/gene'
...
load('./data/RData/ref_tbl_table3.RData')
length(statList)
for(spP in statList){
  statDF = read.csv(paste0(baseDir, '/', spP))
  Mg = spP %>% str_remove('\\.fa.csv') %>% str_remove('inStrain_gene_statistics_')
  sp = bin2rank$Species[bin2rank$ID == Mg]
  ...
  statDF1$gene_name = ref_tbl_table3$target_name[match(statDF1$gene, ref_tbl_table3$query_name)]
  statDF1$gene_name[is.na( statDF1$gene_name)] = 'Uncharacterized'
...
openxlsx::write.xlsx(l, paste0('./data/instrain_gene/MAGs_gene_vs_genome.adj.xlsx'))
# volcano_plot(statDF3, x = 'pi_logFC', y = 'pi_pvalue.adj', FC_cutoff = 1.2)
statDF3_all$Species = bin2rank$Species[match(statDF3_all$Species, bin2rank$ID)]
save(statDF3_all, file = './data/RData/MAGs_statDF3_all.RData')
save(l, file = './data/RData/MAGs_ll.RData')
###### scatter plot for each species, SNVs vs SNS, dN/dS vs pNpS ######
load('./data/RData/MAGs_statDF3_all.RData')
table(statDF3_all$Species)

cairo_pdf(file = './plot/scatterplot_dNdS_vs_pNpS_MAGs.pdf', width = 4.47, height = 4.3, onefile = T)
...
table(statDF3_all$Species)
# statDF3_all = statDF3_all[statDF3_all$Species %in% c(
#   'Bifidobacterium vaginale | Ming_nova_VS_4_54_bwa_bin.2',
#   'Aerococcus christensenii | Ming_nova_VS_2_92_bwa_bin.4', 
#   "Limosilactobacillus vaginalis | Ming_nova_VS_1_13_bwa_bin.1",
#   'Fannyhessea vaginae | Ming_nova_VS_2_83_bwa_bin.2',
#   'Limosilactobacillus portuensis | SF1730_bin.6',
#   
#                                                      "Lactobacillus crispatus | Ming_nova_VS_2_09_bwa_bin.1"      ,
#                                                      "Lactobacillus gasseri | Ming_nova_VS_2_79_bwa_bin.1"  ,      
#                                                      "Lactobacillus iners | Ming_nova_VS_4_46_bwa_bin.6"    ,      
#                                                      "Limosilactobacillus coleohominis_A | SF1559_bin.15"    ,     
#                                                      "Lactobacillus jensenii | SF1730_bin.9" ), ]
names(colorSpecies)
...
if(T){
  ...
  cairo_pdf('./plot/distribution_of_dnds_pnps_genes3_MAGs.pdf', width = 3.94, height = 4.39)
...
# 1. Create the histogram plot
  ...
  # 3. Align the two plots and then overlay them.
  aligned_plots <- cowplot::align_plots(phist, pdensity, align="hv", axis="tblr")
  cairo_pdf('./plot/stat_of_dnds_pnps_genes_include_0_2_MAGs.pdf', width = 3.94, height = 4.39)
  cowplot::ggdraw(aligned_plots[[1]]) + cowplot::draw_plot(aligned_plots[[2]])
  dev.off()
}
...
cairo_pdf('./plot/percentage_of_pnps_bigger1_MAGs.pdf', width = 10, height = 3.74)
print(gs)
dev.off()
###### circular packing to show up and down pi genes ######
load('./data/RData/statDF3_all.RData')
load('./data/RData/ll.RData')
...
length(unique(packingDF$gene))
length(unique(packingDF$Species))
...
if(T){
  ...
  cairo_pdf('./plot/pi_high_low_gene_circle_packing_2.pdf', width = 11.97 , height = 8.35)
  print(cP)
  dev.off()
}
...
table(packingDF$gene) %>% length()
table(packingDF$gene_name) %>% length()
...
cP = ggraph(mygraph, layout = 'circlepack') +  # weight = size
  ...
  theme_void() + theme(legend.text = element_text(size = 13), legend.key.size = unit(0.5, 'cm'), 
                       legend.title = element_text(size = 13), plot.margin = unit(c(0,1,0,0), 'cm'))
cairo_pdf('./plot/pNpS_gene_circle_packing_3.pdf', width = 8.81 , height = 4.76)
print(cP)
dev.off()
...
cP = ggraph(mygraph, layout = 'circlepack') +  # weight = size
  ...
  theme_void() + theme(legend.text = element_text(size = 13), legend.key.size = unit(0.5, 'cm'), 
                       legend.title = element_text(size = 13), plot.margin = unit(c(0,1,0,0), 'cm'))
cairo_pdf('./plot/dNdS_gene_circle_packing_3.pdf', width = 6.92 , height = 4.36)
print(cP)
dev.off()
...
load('./data/RData/MAGs_statDF3_all.RData')
load('./data/RData/MAGs_ll.RData')
statDF3_all = statDF3_all[statDF3_all$Species %in% c("Limosilactobacillus vaginalis | Ming_nova_VS_1_13_bwa_bin.1",
                                                     "Lactobacillus crispatus | Ming_nova_VS_2_09_bwa_bin.1"      ,
                                                     "Lactobacillus gasseri | Ming_nova_VS_2_79_bwa_bin.1"  ,      
                                                     "Lactobacillus iners | Ming_nova_VS_4_46_bwa_bin.6"    ,      
                                                     "Limosilactobacillus coleohominis_A | SF1559_bin.15"    ,     
...
statKey = statDF3_all[statDF3_all$gene %in% ref_tbl_table2.22 & 
                        grepl('actobacillus', statDF3_all$Species) &
                        statDF3_all$sampleNum > 10,]
table(statKey$Species)
nrow(statKey)
myShape = c(22:25)
names(myShape) = keyGenes
statKey$Species2 = str_split_fixed(statKey$Species , '\\|', 2)[,1] %>% 
  str_replace_all(., 'Lactobacillus', 'L.') %>% 
  str_replace_all(., 'Limosilactobacillus', 'L.') %>% 
  str_remove(., '_A') %>% 
  str_trim()
table(statKey$Species2)

names(colorSpecies)  %<>% str_replace_all(., 'Lactobacillus', 'L.')
statKey = statKey[statKey$Species2 %in% names(colorSpecies), ]
statKey$Species2 %<>% factor(., levels = intersect(names(colorSpecies), statKey$Species2))
unique(statKey$Species)
statKey$gene_name %<>% factor(., levels = keyGenes)
sP = ggplot(statKey, aes(x = meanGen_dNdS, y = meanGen_pNpS)) +
  geom_point(aes(color = Species2, fill = meanGen_pi, shape = gene_name, size = NSp_bigger_1_freq), stroke = 1) +
...
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics_MAGs.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()


load('./data/RData/MAGs_geneDF.RData')
avgDF = data.frame()
for(sp in unique(geneDF$genome)){
  geneDF_sub = geneDF[geneDF$genome == sp, ]
  avgDF_sub = geneDF_sub %>% group_by(gene) %>%
    summarise(sampleNum = n(),
              NSp_bigger_1_freq = sum(dNdS[(!is.infinite(dNdS))&(!is.na(dNdS))] > 1),
              meanpi = mean(nucl_diversity[(!is.infinite(nucl_diversity))&(!is.na(nucl_diversity))]),
              meandNdS = mean(dNdS[(!is.infinite(dNdS))&(!is.na(dNdS))]),
              meanpNpS = mean(dNdS[(!is.infinite(dNdS))&(!is.na(dNdS))]))
  avgDF = rbind(avgDF, avgDF_sub)
}
avgDF$NSp_bigger_1_freq =avgDF$NSp_bigger_1_freq/ avgDF_sub$sampleNum
statKey = avgDF[avgDF$gene %in% ref_tbl_table2.22 & 
                  avgDF$sampleNum > 10,]
nrow(statKey)
myShape = c(22:25)
names(myShape) = keyGenes
statKey$Species =  geneDF$Species[match(statKey$gene, geneDF$gene)]
statKey$Species = bin2rank$species[match(statKey$Species, bin2rank$ID)]
statKey$Species = str_split_fixed(statKey$Species , '\\|', 2)[,1] %>% str_replace_all(., 'Lactobacillus', 'L.') %>% str_trim()
names(colorSpecies)  %<>% str_replace_all(., 'Lactobacillus', 'L.')
statKey$Species %<>% factor(., levels = intersect(names(colorSpecies), statKey$Species))
load('./data/RData/ref_tbl_table3.RData')
statKey$gene_name = ref_tbl_table3$target_name[match(statKey$gene, ref_tbl_table3$query_name)]
statKey$gene_name %<>% factor(., levels = keyGenes)
sP = ggplot(statKey, aes(x = meandNdS, y = meanpNpS)) +
  geom_point(aes(color = Species, fill = meanpi, shape = gene_name, size = NSp_bigger_1_freq), stroke = 1) +
  geom_hline(yintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  geom_vline(xintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  scale_shape_manual(values = myShape, name = 'Gene') +
  # scale_fill_manual(values = colorSpecies) +
...
cairo_pdf(file =  './plot/gene_for_each_species_evolutionary_metrics.pdf', width = 11.14, height = 7.14, onefile = T)
for(sp in unique(statDF3_all$Species)){
...
cairo_pdf('./plot/dnds_freq_barplot.pdf', width = 5.21, height = 5.60, onefile = T)
print(dP)
print(dPs2)
print(dPs3)
dev.off()
...
cairo_pdf('./plot/dnds_bigger1_barplot.pdf', width = 5.21, height = 5.60, onefile = T)
print(dP)
print(dP2)
print(dP3)
dev.off()
