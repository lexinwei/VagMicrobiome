...
openxlsx::write.xlsx(l, paste0('./data/instrain_gene/gene_vs_genome.adj.xlsx'))
# volcano_plot(statDF3, x = 'pi_logFC', y = 'pi_pvalue.adj', FC_cutoff = 1.2)
save(statDF3_all, file = './data/RData/statDF3_all.RData')
save(l, file = './data/RData/ll.RData')
###### scatter plot for each species, SNVs vs SNS, dN/dS vs pNpS ######
load('./data/RData/statDF3_all.RData')
cairo_pdf(file = './plot/scatterplot_dNdS_vs_pNpS.pdf', width = 4.47, height = 4.3, onefile = T)
...
write.xlsx(geneDF_to_paper, file = './table/gene_evoluionary_metrices_this_study.xlsx')
...
if(T){
  ...
  cairo_pdf('./plot/distribution_of_dnds_pnps_genes2.pdf', width = 3.94, height = 4.39)
...
# 1. Create the histogram plot
  ...
  # 3. Align the two plots and then overlay them.
  aligned_plots <- cowplot::align_plots(phist, pdensity, align="hv", axis="tblr")
  cairo_pdf('./plot/stat_of_dnds_pnps_genes_include_0_2.pdf', width = 3.94, height = 4.39)
  cowplot::ggdraw(aligned_plots[[1]]) + cowplot::draw_plot(aligned_plots[[2]])
  dev.off()
}
...
cairo_pdf('./plot/percentage_of_pnps_bigger1.pdf', width = 10, height = 3.74)
print(gs)
dev.off()
...
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
...
load('./data/RData/colorList.RData')
statKey = geneDF %>% group_by(Species, gene) %>%
  summarise(meanGen_dNdS = mean(dNdS, na.rm = T),
            meanGen_pNpS = mean(pNpS, na.rm = T),
            meanGen_pi = mean(nucl_diversity, na.rm = T),
            NSp_bigger_1_freq = sum(pNpS[!is.na(pNpS)] > 1),
            N = n(),
            vaid_pnps = sum(!is.na(pNpS)),
            vaid_dnds = sum(!is.na(dNdS)))
statKey$NSp_bigger_1_freq = statKey$NSp_bigger_1_freq/statKey$N
statKey$gene_name = geneDF$gene_name[match(statKey$gene, geneDF$gene)]
myShape = c(22:25)
names(myShape) = keyGenes
statKey$Species %<>% str_replace_all(., 'Lactobacillus', 'L.')
names(colorSpecies)  %<>% str_replace_all(., 'Lactobacillus', 'L.')
...
nrow(statKey)
statKey = statKey[statKey$gene_name %in% keyGenes,]
sP = ggplot(statKey[statKey$vaid_pnps > 5, ], aes(x = meanGen_dNdS, y = meanGen_pNpS)) +
  geom_point(aes(color = Species, fill = meanGen_pi, shape = gene_name, size = NSp_bigger_1_freq), stroke = 1) +
  geom_hline(yintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  geom_vline(xintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  scale_shape_manual(values = myShape, name = 'Gene') +
...
# ggplot(statKey, aes(x = meanGen_pNpS, y = meanGen_pi)) +
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics_new.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
