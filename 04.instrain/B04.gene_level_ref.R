...
if(F){
  ...
  for (sp in dataList) {
    ...
    gene_df$Species = sp %>% str_remove('inStrain_gene_') %>%
      str_remove('\\.fa\\.csv')
  ...
  save(geneDF, file = './data/RData/MAGs_geneDF2.RData')
}


######### evolutionary metrics for each gene (pN/pS)  along genomes loci #####
load('./data/RData/MAGs_geneDF2.RData')
table(geneDF$Species)
...
pwPNPS2_avg = pwPNPS2_avg[pwPNPS2_avg$speciesNum >= 5, ]
...
if(T){
  ...
  ht = Heatmap(ptMat, name = "pN/pS of KEGG pathway", col = col_fun, 
               ...
               show_row_names = T, column_names_rot = 45, column_names_centered = F, 
               row_title = NULL, 
               heatmap_legend_param = list(direction = "horizontal", border = '#898B8E',
                                           legend_width = unit(4, "cm"), title_position = 'topcenter'))
  cairo_pdf('./plot/KEGG_pathway_top20_down20.pdf', height = 9.46, width = 10)
