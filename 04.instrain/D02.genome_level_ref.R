...
load('./data/RData/PRJEB34536_refDF_minCovBreath10.RData')
...
if(F){
  ...
  if(T){
    ...
    write.xlsx(refMetData2_paper, file = './table/PRJEB34536_evoluionary_metrices_validation.xlsx')
    
    refMetData2_2 = refMetData2[refMetData2$species %in% names(colorSpecies), ]
    dim(refMetData2_2)
    cairo_pdf('./plot/PRJEB34536_SNVs_vs_D_vs_coverage_vs_pNpS_3.pdf', width = 8, height = 3.61, onefile = T)
  ...
  if(T){
    cairo_pdf('./plot/PRJEB34536_pi_vs_pNpS_vs_coverage_vs_D.pdf', width = 8, height = 3.63, onefile = T)
  ...
  if(T){
    ...
    wx5 = ggplot(pnpsDF2, aes(x = mean_pi, y = mean_pNpS, size = mean_coverage, fill = Family)) +
      ...
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/PRJEB34536_evo_type3.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx5)
    dev.off()
  }
  #### one vs one ####
...
if(T){
  ...
  useData2 = melt(useData, id.vars = c('genome_species', 'if_Lactobacillaceae'))
...
if(T){
  cairo_pdf('./plot/PRJEB34536_evolution_metrics_nonLac_vs_Lac3.pdf', width = 10.47, height = 1.78)
  stat.test = useData3 %>% 
    group_by(Metrics) %>% 
...
if(T){
  cairo_pdf('./plot/PRJEB34536_evolution_metrics_nonLac_vs_Lac_facetBy_Ethnicity3.pdf', width = 10.85, height = 4.69)
