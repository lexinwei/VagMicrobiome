...
if(F){
  sampleData0 = read.xlsx('./public_cohort/!!!PRJEB34536_npj_biofilm/filereport_read_run_PRJEB34536_tsv.xlsx')
  sampleData1 = sampleData0
  table(sampleData1$sample_alias)
  metaData2 = sampleData1
}
...
if(T){
  ...
  for(ss in sampleData1$run_accession){
    print(ss)
    geneDat = read.delim2(paste0('./instrain_res/PRJEB34536/gene_info/', ss, '.IS_gene_info.tsv'))
    dt = fread(paste0('./instrain_res/PRJEB34536/genes_SNP_count/', ss, '_genes_SNP_count.csv.gz'), 
    ...
    genomeDat = read.delim2(paste0('./instrain_res/PRJEB34536/genome_info/', ss, '.IS_genome_info.tsv'))
  ...
  refDF$pt_ID.u = metaData2$sample_title[match(refDF$sample, metaData2$run_accession)]
...
save(refDF, file = './data/RData/PRJEB34536_refDF_minCovBreath10.RData')
...
refVarDF = refDF[, c('species', 'pt_ID.u', 'coverage_log10', 'breadth', 'SNPs_per_bp', 'nucl_diversity', 'dn_ds_common']
