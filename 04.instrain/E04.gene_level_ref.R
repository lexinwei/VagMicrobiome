...
if(F){
  ...
  save(geneDF, file = './data/RData/PRJNA288562_geneDF.RData')
}
...
cairo_pdf(file =  './plot/PRJEB34536_gene_for_each_species_evolutionary_metrics_pNpS_facet_byEthniticy.pdf', width = 11.14, height = 4, onefile = T)
...
cairo_pdf(file =  './plot/PRJEB34536_gene_for_each_species_evolutionary_metrics_2.pdf', width = 7.09, height = 3.49, onefile = T)
for(sp in unique(geneDF$Species)){
...
load('./data/RData/PRJEB34536_geneDF.RData')
table(geneDF$Species)
a  = geneDF[grep('muc', geneDF$gene_description, ignore.case = T), c('gene_name', 'gene_description')]
a = a[!duplicated(a), ]
keyGenes = c('DDE_Tnp_1', 'DDE_Tnp_IS66', 'Gram_pos_anchor',
...
myShape = c(22:25)
names(myShape) = keyGenes
cairo_pdf(file =  './plot/PRJEB34536_gene_for_each_species_evolutionary_metrics_dNdS_facet_byEthniticy.pdf', 
...
cairo_pdf(file =  './plot/PRJEB34536_gene_for_each_species_evolutionary_metrics_2_dNdS.pdf', width = 7.09, height = 3.49, onefile = T)
for(sp in unique(geneDF$Species)){
  geneDF_sub = geneDF[geneDF$Species == sp, ]
  length(unique(geneDF_sub$sample))
  length(unique(geneDF_sub$gene))
