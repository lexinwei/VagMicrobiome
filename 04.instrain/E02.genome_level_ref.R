...
load('./data/RData/PRJNA288562_refDF_minCovBreath10.RData')
...
refMetData2$coverage %<>% as.numeric()
...
if(F){
  ...
  if(T){
    ...
    write.xlsx(refMetData2_paper, file = './table/PRJNA288562_evoluionary_metrices_validation.xlsx')
    
    refMetData2_2 = refMetData2[refMetData2$species %in% names(colorSpecies), ]
    dim(refMetData2_2)
    cairo_pdf('./plot/PRJNA288562_SNVs_vs_D_vs_coverage_vs_pNpS_3.pdf', width = 8, height = 3.61, onefile = T)
  ...
  if(T){
    cairo_pdf('./plot/PRJNA288562_pi_vs_pNpS_vs_coverage_vs_D.pdf', width = 8, height = 3.63, onefile = T)
  ...
  if(T){ # overview: x = SNVs/kbp , y = D', colored by pN/pS, size by coverage, shape by species group
    cairo_pdf('./plot/PRJEB34536_SNVs_vs_D_vs_coverage_vs_pNpS_facetBy_Ethnicity.pdf', width = 10.28, height = 4.48)
  ...
  if(T){
    ...
    wx5 = ggplot(pnpsDF2, aes(x = mean_pi, y = mean_pNpS, size = mean_coverage, fill = Family)) +
      ...
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/PRJNA288562_evo_type3.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx5)
    dev.off()
  }
  #### one vs one ####
  if(T){ # overview: x = Pi , y = D', colored by pN/pS, size by coverage, shape by species group
    cairo_pdf('./plot/PRJEB34536_evolutionary_metrics_one_vs_one.pdf', width = 8, height = 3, onefile = T)
  ...
  cairo_pdf('./plot/PRJEB34536_species_pi_vs_pnps.pdf', width = 20, height = 10)
  relPlot(refMetData2, xval = 'nucl_diversity', zval = 'genome_species',xAngle = 30,
          ...
          gval = 'if_Lactobacillaceae', mycolor = colorLactobacillaceae)
  dev.off()
  cairo_pdf('./plot/PRJEB34536_species_pi_vs_pnps_iners.pdf', width = 8, height = 3, onefile = T)
  ...
  dev.off()
  cairo_pdf('./plot/PRJEB34536_species_pi_vs_pnps_iners_facetBy_Ethnicity.pdf', width = 11.48, height = 2.52, onefile = T)
  ...
  if(T){
    cairo_pdf('./plot/PRJEB34536_evolutionary_metrics_one_vs_one_facetBy_Ethnicity.pdf', width = 11.48, height = 2.52, onefile = T)
...
cairo_pdf(file = './plot/PRJEB34536_evolutionary_metrics_one_vs_one_facetBy_Ethnicity_pi_vs_pnps.pdf', width = 11.51, height = 4)
...
cairo_pdf(file = './plot/PRJEB34536_evolutionary_metrics_one_vs_one_facetBy_Ethnicity_pi_vs_pnps3.pdf', 
...
if(T){
  ...
  cairo_pdf('./plot/PRJEB34536_relationship among evolutionary metrics by each family.pdf', width = 12.78, height = 2.8)
  print(myP)
  dev.off()
}
...
if(T){
  ...
  myP = ggplot(compDF, aes(x=Group, y=Slope)) +
    ...
    mytheme3 + theme(strip.text = element_text(size = 13), strip.background = element_rect(fill = '#E2E5E9', color = NA),
                     axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
                     legend.position = 'right')
  cairo_pdf('./plot/PRJEB34536_relationship among evolutionary metrics by each species.pdf', width = 13.36, height = 7.33)
  print(myP)
  dev.off()
}
...
if(T){
  ...
  myP = ggplot(compDF, aes(x=Group, y=PearsonR)) +
    ...
    mytheme3 + theme(strip.text = element_text(size = 13), strip.background = element_rect(fill = '#E2E5E9', color = NA),
                     # axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
                     legend.position = 'right')
  cairo_pdf('./plot/PRJEB34536_relationship among evolutionary metrics by each species_2.pdf', width = 13.15, height = 7.33)
  print(myP)
  dev.off()
}
######## ggpair ######
if(T){
  ...
  cairo_pdf('./plot/PRJEB34536_corr_of_diversity_vs_coverage_3.pdf', height = 8.18, width = 8.03, onefile = T)
...
if(T){
  dim(refMetData2)
  refMetData2$Ethnicity
  refMetData2$Race
  range(refMetData2$D_rev, na.rm = T)
  ys = c('genome_species', 'if_Lactobacillaceae', 
         'SNV_per_kbp', 'D_rev', 'nucl_diversity', 'pnps',
         'Ethnicity', 'Race')
  useData = refMetData2[, ys]
  useData2 = melt(useData, id.vars = c('genome_species', 'if_Lactobacillaceae','Ethnicity', 'Race'))
  colnames(useData2) = c('Species', 'Group', 'Ethnicity', 'Race','Metrics', 'Value')
  ...
  useData3 = useData2[!(useData2$Metrics == "1 - D'" & 
                        useData2$Value >= 0.2), ]
  useData3 = useData3[!is.infinite(useData3$Value), ]
  useData3 = na.omit(useData3)
  range(useData3$Value)
  useData3$Value[useData3$Value < 0] = 0
}
if(T){
  cairo_pdf('./plot/PRJNA288562_evolution_metrics_nonLac_vs_Lac4.pdf', width = 3.66, height = 3.04)
  stat.test = useData3 %>% 
    group_by(Metrics) %>% 
  ...
  stat.test$y.position = c(10, 0.015, 0.125, 1.9)
...
if(T){
  cairo_pdf('./plot/PRJNA288562_evolution_metrics_nonLac_vs_Lac_facetBy_Ethnicity3.pdf', 
            width = 10.85, height = 4.69)
  ...
  stat.test2$y.position = c(rep(8, 2),
                           rep(0.01, 2),
                           rep(0.125, 2),
                           rep(1.5, 2))
...
if(T){
  cairo_pdf('./plot/PRJNA288562_evolution_metrics_nonLac_vs_Lac_facetBy_Ethnicity3.pdf', width = 10.85, height = 4.69)
  useData3$Ethnicity = useData3$Race
  cDF = useData3 %>% group_by(Metrics, Ethnicity) %>% summarise(N = n())
  table(useData3$Ethnicity, useData3$Metrics)
  useData3.1 = useData3[(useData3$Ethnicity == 'Asian-Chinese' & useData3$Metrics == "1 - D'"), ]
  useData3 = useData3[!(useData3$Ethnicity == 'Asian-Chinese' & useData3$Metrics == "1 - D'"), ]
  stat.test2 <- useData3 %>% group_by(Metrics, Ethnicity) %>%
    wilcox_test(Value ~ Group) %>%
    adjust_pvalue(method = "BH") %>%
    add_significance("p.adj") %>%
    add_xy_position(x = 'Group', dodge = 0)
  stat.test2$y.position = c(rep(8, 4),
                            rep(0.01, 4),
                            rep(0.125, 3),
                            rep(1.5, 4))
  stat.test2$p.adj %<>% signif(., digits = 3)
...
if(T){
  ...
  cairo_pdf('./plot/PRJEB34536_evolution_metrics_Ethnicity_By_nonLac_vs_Lac_2.pdf', width = 5.09, height = 3.46, onefile = T)
...
if(T){
  ...
  pp = p %>% facet(nrow = 1, facet.by = 'Metrics', scales = 'free_x') +
    ...
    coord_flip()
  cairo_pdf('./plot/PRJEB34536_evolution_metrics_nonLac_vs_Lac.pdf', width = 9.78, height = 1.78)
  print(pp)
  dev.off()
}
...
if(T){
  ...
  if(plotFlag){
    openxlsx::write.xlsx(comp_DF, file = './data/PRJEB34536_microdiversity_pairwise_group_comp_3.xlsx')  
    openxlsx::write.xlsx(uniRes, file = './data/PRJEB34536_microdiversity_uni_regression_v3.xlsx')
  }
}
...
# merge
uniRes = readxl::read_xlsx('./data/PRJEB34536_microdiversity_uni_regression_v3.xlsx')
...
openxlsx::write.xlsx(res_mulisig, './data/PRJEB34536_microdiveristy_lme_regression_fixed_sig_v3.xlsx')

##### plot boxplot for candidate species and variable all references and mags #####
plotFlag = T
if(plotFlag){
  pdf('./plot/PRJEB34536_microdiveristy_boxplot_all_cov10.pdf', width = 10, height = 7)
}
x = 'Ethnicity'
y = 'nucl_diversity'
...
if(plotFlag){
  pdf('./plot/PRJEB34536_microdiveristy_super_sig_2_v3_cov10.pdf', width = 10, height = 7)
}
...
cairo_pdf('./plot/PRJEB34536_microdiversity_super_v3.pdf', width = 4.34,height = 3.55, onefile = T)
if(T){
...
##### carefully!! ####
cairo_pdf('./plot/PRJEB34536_microdiversity_super_good_results.pdf', width = 6.05,height = 3.55, onefile = T)
if(T){
  sp = 'Fannyhessea vaginae'
  tmpDF = refMetData[refMetData$genome_species == sp, ]
  tmpDF$Trimester = tmpDF$trimester
...
###### carefully !!! #####
if(T){
  cairo_pdf('./plot/PRJEB34536_microdiversity_super_good_results2.pdf', width = 6.95,height = 3.61, onefile = T)
  sp = 'Gardnerella vaginalis'
  tmpDF = refMetData[refMetData$genome_species == sp, ]
...
table(col_fun$col)
library(circlize)
cairo_pdf('./plot/PRJEB34536_circos_microdiveristy_v3.pdf', width = 6.25, height = 8.52)
...
cairo_pdf('./plot/PRJEB34536_lme_microdiversity_2.pdf', width = 4.08, height = 3.01, onefile = T)
if(T){
...
cairo_pdf('./plot/PRJEB34536_gamm_microdiveristy.pdf', width = 4.66, height = 3.26, onefile = T)
if(T){
  refMetData$Term2 = refMetData$Term_char2
  refMetData$Sample_GA = swabData$Sample_GA[match(refMetData$pt_ID.u, swabData$pt_ID.u)]
  ys = c('nucl_diversity', 'dnds', 'pnps', 'dn_ds_common')
...
table(col_fun$col)
library(circlize)
pdf('./plot/PRJEB34536_circos_association_genitic_diversity_best.pdf', width = 17, height = 15)
