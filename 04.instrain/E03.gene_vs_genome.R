...
baseDir = './instrain_res/PRJNA288562/gene'
...
openxlsx::write.xlsx(l, paste0('./data/instrain_gene/PRJNA288562_gene_vs_genome.adj.xlsx'))
# volcano_plot(statDF3, x = 'pi_logFC', y = 'pi_pvalue.adj', FC_cutoff = 1.2)
save(statDF3_all, file = './data/RData/PRJNA288562_statDF3_all.RData')
save(l, file = './data/RData/PRJNA288562_ll.RData')
###### scatter plot for each species, SNVs vs SNS, dN/dS vs pNpS ######
load('./data/RData/PRJNA288562_statDF3_all.RData')
cairo_pdf(file = './plot/PRJNA288562_scatterplot_dNdS_vs_pNpS.pdf', 
          width = 4.47, height = 4.3, onefile = T)
...
load('./data/RData/PRJNA288562_ll.RData')
...
if(T){
  ...
  cairo_pdf('./plot/PRJNA288562_distribution_of_dnds_pnps_genes.pdf', width = 3.94, height = 4.39)
...
# 1. Create the histogram plot
  ...
  # 3. Align the two plots and then overlay them.
  aligned_plots <- cowplot::align_plots(phist, pdensity, align="hv", axis="tblr")
  cairo_pdf('./plot/PRJNA288562_stat_of_dnds_pnps_genes_include_0.pdf', width = 3.94, height = 4.39)
  cowplot::ggdraw(aligned_plots[[1]]) + cowplot::draw_plot(aligned_plots[[2]])
  dev.off()
}
...
statKey = statDF3_all[statDF3_all$gene_name %in% keyGenes & statDF3_all$sampleNum > 3,]
nrow(statKey)
myShape = c(22:25)
...
#   geom_point()
cairo_pdf('./plot/PRJNA288562_key_genes_evolutionary_metrics.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()

load('./data/RData/PRJNA288562_geneDF.RData')
...
sP = ggplot(statKey[statKey$vaid_pnps >=1, ], aes(x = meanGen_dNdS, y = meanGen_pNpS)) +
  geom_point(aes(color = Species, fill = meanGen_pi, shape = gene_name, size = NSp_bigger_1_freq), stroke = 1) +
  geom_hline(yintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  geom_vline(xintercept = 1, linewidth = 0.3, linetype = 'dashed', color = 'gray') +
  scale_shape_manual(values = myShape, name = 'Gene') +
...
# ggplot(statKey, aes(x = meanGen_pNpS, y = meanGen_pi)) +
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics_PRJNA288562_new.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
