...
lolPlot = function(., type = 'AA', keep_mut_on_domain = F){ # or type = 'NT'
  ...
  snvDF$pt_ID.u = swabData$pt_ID.u[match(snvDF$seq_ID, swabData$seq_ID)]
  snvDF$pt_ID = swabData$pt_ID[match(snvDF$seq_ID, swabData$seq_ID)]
  snvDF$Term = metaData$Term_char2[match(snvDF$pt_ID, metaData$pt_ID)]
  snvDF$Ethnicity = metaData$Ethnicity[match(snvDF$pt_ID, metaData$pt_ID)]
  pretermCount = snvDF %>% group_by(mutation) %>% dplyr::count(Term == 'Preterm')
  pretermCount = pretermCount[pretermCount$`Term == "Preterm"`, ]
  ...
  snvCountDF$pretermCount = pretermCount$n[match(snvCountDF$mutation, pretermCount$mutation)]
  snvCountDF$pretermFreq = snvCountDF$pretermCount/snvCountDF$Counts
  ...
  cairo_pdf(paste0('./plot/lol_mutation_', species, '_', geneID, '_', type, fileN, '.pdf'), width = 9.65, height = 6.43)
...
lolPlot2 = function(., type = 'AA', keep_mut_on_domain = F,pdfHei = 3.44){ # or type = 'NT'
  ...
  cairo_pdf(paste0('./plot/bilol_mutation_', species, '_', geneID, '_', type, fileN, '.pdf'), 
            width = 9.65, height = pdfHei, onefile = T) # 6.43
  ...
  ht = Heatmap( t(convScore[, 'Score'] ), name = 'Conservation score', 
                col = colorRampPalette(brewer.pal(8, "PuBuGn"))(100),
...
if(T){ # AA
  ...
  lolPlot2(type = 'AA', keep_mut_on_domain = F)
}
##### L. crisptus, Gram_pos_anchor, NZ_CP039266.1_48 #####
if(T){ # NT
  library(trackViewer)
...
if(F){
  snvDF = read.csv('./instrain_res/s__Lactobacillus_iners.fasta_NZ_CP045664.1_439.csv')
  ...
  pretermCount = snvDF %>% group_by(mutation) %>% dplyr::count(Term == 'Preterm')
  pretermCount = pretermCount[pretermCount$`Term == "Preterm"`, ]
  snvCountDF = read.csv('./instrain_res/s__Lactobacillus_iners.fasta_NZ_CP045664.1_439_stat.csv')
  snvCountDF$NT_pos = snvDF$position[match(snvCountDF$mutation, snvDF$mutation)]
  snvCountDF$gene_pos <- str_extract(snvCountDF$mutation, '\\d+') %>% as.numeric()
  range(snvCountDF$gene_pos)
  snvCountDF$mutation_type = snvDF$mutation_type[match(snvCountDF$mutation, snvDF$mutation)]
  snvCountDF$pretermCount = pretermCount$n[match(snvCountDF$mutation, pretermCount$mutation)]
  snvCountDF$pretermFreq = snvCountDF$pretermCount/snvCountDF$Counts*100
  library(trackViewer)
...
if(F){
  snvDF = read.csv('./instrain_res/s__Lactobacillus_crispatus.fasta_NZ_CP039266.1_1431.csv')
  snvDF$gene_pos <- str_extract(snvDF$mutation, '\\d+') %>% as.numeric()
  snvDF$seq_ID = str_extract(snvDF$sample, '\\d+_\\d+')
  snvDF$pt_ID.u = swabData$pt_ID.u[match(snvDF$seq_ID, swabData$seq_ID)]
  snvDF$pt_ID = swabData$pt_ID[match(snvDF$seq_ID, swabData$seq_ID)]
  ...
  pretermCount = snvDF %>% group_by(mutation) %>% dplyr::count(Term == 'Preterm')
  pretermCount = pretermCount[pretermCount$`Term == "Preterm"`, ]
  snvCountDF = read.csv('./instrain_res/s__Lactobacillus_crispatus.fasta_NZ_CP039266.1_1431_stat.csv')
  snvCountDF$gene_pos <- str_extract(snvCountDF$mutation, '\\d+') %>% as.numeric()
  snvCountDF$NT_pos = snvDF$position[match(snvCountDF$mutation, snvDF$mutation)]
