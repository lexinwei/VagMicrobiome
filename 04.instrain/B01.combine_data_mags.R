...
# read stb file
baseDir='./instrain_res/to_MAGs_139_2/'
if(T){
  refSTB = read.delim2(paste0(baseDir, '/mags_139_genomes2.stb'), sep = '\t', header = F, col.names = c('scaffold', 'genome'))
  refDF = data.frame()
  for(ss in swabData$fqID){
    print(ss)
    geneDat = read.delim2(paste0(baseDir, '/gene_info/', ss, '_bwa.IS_gene_info.tsv'))
    dt = fread(paste0(baseDir, '/genes_SNP_count/', ss, '_bwa_genes_SNP_count.csv.gz'), 
    ...
    genomeDat = read.delim2(paste0(baseDir, '/genome_info/', ss, '_bwa.IS_genome_info.tsv'))
  ...
  refDF$SNV_count %<>% as.numeric()
  refDF$SNS_count %<>% as.numeric()
  refDF$SNVs_per_bp = refDF$SNV_count/(refDF$length*refDF$breadth_minCov)
  refDF$SNSs_per_bp = refDF$SNS_count/(refDF$length*refDF$breadth_minCov)
  refDF$SNPs_per_bp = (refDF$SNV_count+refDF$SNS_count)/(refDF$length*refDF$breadth_minCov)
}
save(refDF, file = './data/RData/refDF_mags.RData')
...
refDF$relative_abundance = magAbuDF$relative_abundance[match(paste0(refDF$species, '|', refDF$pt_ID.u), 
...
if(plotFlag){
  pdf('./plot/mags_genome_info.pdf', width = 15, height = 8)
  labDF = data.frame(
