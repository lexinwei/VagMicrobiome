...
baseDir = './instrain_res/to_NCBI_ref2/gene'
...
openxlsx::write.xlsx(l, paste0('./data/instrain_gene/gene_vs_genome_RE.adj.xlsx'))
# volcano_plot(statDF3, x = 'pi_logFC', y = 'pi_pvalue.adj', FC_cutoff = 1.2)
save(statDF3_all, file = './data/RData/statDF3_all_RE.RData')
save(l, file = './data/RData/ll_RE.RData')
###### scatter plot for each species, SNVs vs SNS, dN/dS vs pNpS ######
load('./data/RData/statDF3_all_RE.RData')
cairo_pdf(file = './plot/scatterplot_dNdS_vs_pNpS_RE.pdf', width = 4.47, height = 4.3, onefile = T)
...
write.xlsx(geneDF_to_paper, file = './table/gene_evoluionary_metrices_this_study_RE.xlsx')

load('./data/RData/ll_RE.RData')
...
if(T){
  ...
  cairo_pdf('./plot/distribution_of_dnds_pnps_genes2_RE.pdf', width = 3.94, height = 4.39)
...
# 1. Create the histogram plot
  ...
  # 3. Align the two plots and then overlay them.
  aligned_plots <- cowplot::align_plots(phist, pdensity, align="hv", axis="tblr")
  cairo_pdf('./plot/stat_of_dnds_pnps_genes_include_0_2_RE.pdf', width = 3.94, height = 4.39)
  cowplot::ggdraw(aligned_plots[[1]]) + cowplot::draw_plot(aligned_plots[[2]])
  dev.off()
}
...
#   geom_point()
cairo_pdf('./plot/key_genes_evolutionary_metrics_RE.pdf', width = 5.80, height = 6.79)
print(sP)
dev.off()
