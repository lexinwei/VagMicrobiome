...
length(unique(refMetData$pt_ID.u))
...
if(F){
  ...
  if(T){
    ...
    cairo_pdf('./plot/SNVs_vs_D_vs_coverage_vs_pNpS_3.pdf', width = 8, height = 3.63, onefile = T)
    dim(refMetData2)
    ...
    paper_var = c('SampleID', 'genome_species', 'Family', 
                  'SNV_per_kbp', 'divergent_site_count', 'length', 'coverage', 'breadth', 'breadth_minCov', 
                  'nucl_diversity', 'pnps', 'D_rev')
    paper_col = c('SampleID', 'Genome', 'Family',
                  'SNVs/kbp', 'SNPs count', 'Length', 'Coverage', 'Breadth', 'Breadth_minCov',
                  'π', 'pN/pS', "1-D'")
    sum(refMetData2$SNV_per_kbp)
    sum(refMetData2$SNS_per_kbp)
    refMetData2_paper = refMetData2[, paper_var]
    colnames(refMetData2_paper) = paper_col
    write.xlsx(refMetData2_paper, file = './table/evoluionary_metrices_this_study.xlsx')
  ...
  if(T){
    cairo_pdf('./plot/pi_vs_pNpS_vs_coverage_vs_D.pdf', width = 8, height = 3.63, onefile = T)
    ...
    wx4 = ggplot(pnpsDF, aes(x = mean_pi, y = mean_pNpS, size = mean_D, fill = Family)) +
      geom_vhlines(xintercept = mean(pnpsDF$mean_pi), yintercept = mean(pnpsDF$mean_pNpS)) +
      ...
      scale_size_continuous(range = c(2,8), name = "Avg. 1-D'") +
      geom_text_repel(aes(label = Species), size = 4) +
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/evo_type.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx4)
    dev.off()
    ...
    wx5 = ggplot(pnpsDF, aes(x = mean_pi, y = mean_pNpS, size = mean_coverage, fill = Family)) +
      ...
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/evo_type3.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx5)
    dev.off()
  }
  if(T){ # overview: x = SNVs/kbp , y = D', colored by pN/pS, size by coverage, shape by species group
    cairo_pdf('./plot/SNVs_vs_D_vs_coverage_vs_pNpS_facetBy_Ethnicity.pdf', width = 10.28, height = 4.48)
  ...
  if(T){ # overview: x = Pi , y = D', colored by pN/pS, size by coverage, shape by species group
    cairo_pdf('./plot/evolutionary_metrics_one_vs_one_2.pdf', width = 8, height = 3.63, onefile = T)
  ...
  cairo_pdf('./plot/species_pi_vs_pnps.pdf', width = 20, height = 10)
  relPlot(refMetData2, xval = 'nucl_diversity', zval = 'genome_species',xAngle = 30,
          ...
          gval = 'if_Lactobacillaceae', mycolor = colorLactobacillaceae)
  dev.off()
  cairo_pdf('./plot/species_pi_vs_pnps_iners.pdf', width = 8, height = 3, onefile = T)
  for(sp in levels(refMetData2$genome_species)){
    print(sp)
  ...
  dev.off()
  cairo_pdf('./plot/species_pi_vs_pnps_iners_facetBy_Ethnicity.pdf', width = 11.48, height = 2.52, onefile = T)
  relPlot(refMetData2[refMetData2$genome_species == 'Lactobacillus iners',], zval = 'Ethnicity2',
  ...
  if(T){
    cairo_pdf('./plot/evolutionary_metrics_one_vs_one_facetBy_Ethnicity.pdf', width = 11.48, height = 2.52, onefile = T)
    ...
    relPlot(refMetData2, xval = 'coverage', yval = 'pnps', zval = 'Ethnicity2',xLab = 'Coverage', yLab = "pN/pS",
            gval = 'if_Lactobacillaceae', mycolor =colorLactobacillaceae, xAngle = 45)
    relPlot(refMetData2, xval = 'SNV_per_kbp', yval = 'd_prime_mean', zval = 'Ethnicity2', xLab = 'SNVs/kbp', y = "D'",
            gval = 'Family', mycolor = colorFamily, showSE = F)
    relPlot(refMetData2, xval = 'SNV_per_kbp', yval = 'D_rev', zval = 'Ethnicity2', xLab = 'SNVs/kbp', y = "1 - D'",
            gval = 'Family', mycolor = colorFamily, showSE = F)
    relPlot(refMetData2, xval = 'nucl_diversity', yval = 'd_prime_mean', zval = 'Ethnicity2',xLab = expression(italic(pi)), yLab = "D'", 
...
cairo_pdf(file = './plot/evolutionary_metrics_one_vs_one_facetBy_Ethnicity_pi_vs_pnps.pdf', width = 11.51, height = 4)
...
cairo_pdf(file = './plot/evolutionary_metrics_one_vs_one_facetBy_Ethnicity_pi_vs_pnps2.pdf', 
          width = 6.66, height = 6.57)
relPlot(refMetData2, xval = 'nucl_diversity', yval = 'pnps', zval = 'Ethnicity2',
...
if(T){
  ...
  cairo_pdf('./plot/relationship among evolutionary metrics by each family.pdf', width = 12.78, height = 2.8)
  print(myP)
  dev.off()
}
allFQ = read.table('./sample.list')
allFQ$newID = swabData$SampleID.u[match(allFQ$V1, paste0(swabData$fqID, '_bwa'))]
allFQ = allFQ[order(allFQ$V1),]
allFQ$newID[is.na(allFQ$newID)] = paste0('X', str_pad(1:sum(is.na(allFQ)), width = 2, pad = '0'))
allFQ = allFQ[order(allFQ$newID), ]
write.table(allFQ, file = './table/rename_fqID.txt', col.names = F, row.names = F, quote = F)
allFQ$File = paste0('/vag_microbiome/', allFQ$newID)
allFQ$Description = paste0('Human vaginal microbiome')
...
if(T){
  ...
  myP = ggplot(compDF, aes(x=Group, y=Slope)) +
    ...
    mytheme3 + theme(strip.text = element_text(size = 13), strip.background = element_rect(fill = '#E2E5E9', color = NA),
                     axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
                     legend.position = 'right')
  cairo_pdf('./plot/relationship among evolutionary metrics by each species.pdf', width = 13.36, height = 3.99)
  print(myP)
  dev.off()
}
...
if(T){
  ...
  myP = ggplot(compDF, aes(x=Group, y=PearsonR)) +
    ...
    mytheme3 + theme(strip.text = element_text(size = 13), strip.background = element_rect(fill = '#E2E5E9', color = NA),
                     # axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
                     legend.position = 'right')
  cairo_pdf('./plot/relationship among evolutionary metrics by each species_2.pdf', width = 13.15, height = 4.33)
  print(myP)
  dev.off()
}
######## ggpair ######
if(T){
  ...
  cairo_pdf('./plot/corr_of_diversity_vs_coverage_3.pdf', height = 8.18, width = 8.03, onefile = T)
...
if(T){
  cairo_pdf('./plot/evolution_metrics_nonLac_vs_Lac2.pdf', width = 10.47, height = 1.78, onefile = T)
  ...
  p = ggboxplot(useData2, x = 'Group', y = 'Value', fill = 'Group', add = 'jitter', add.params = list(size = 0.1))
  pp = p %>% facet(nrow = 1, facet.by = 'Metrics', scales = 'free_x') +
    stat_pvalue_manual(stat.test, label = "p.adj", tip.length = 0.01,coord.flip = T) +
    mytheme + theme(strip.text = element_text(size = 13)) +
    scale_y_continuous(expand = expansion(mult = c(0.05, 0.13))) + 
...
if(T){
  cairo_pdf('./plot/evolution_metrics_species.pdf', width = 12.36, height = 3.85, onefile = T)
...
dev.off()
if(T){
  cairo_pdf('./plot/evolution_metrics_species_Ethnicity2.pdf', width = 10.85, height = 4.69, onefile = T)
...
if(T){
  cairo_pdf('./plot/evolution_metrics_nonLac_vs_Lac_facetBy_Ethnicity2.pdf', width = 10.85, height = 4.69, onefile = T)
  stat.test2 <- useData2 %>% group_by(Metrics, Ethnicity) %>% 
    wilcox_test(Value ~ Group) %>% 
    adjust_pvalue(method = "BH") %>%
    add_significance("p.adj") %>%
...
if(T){
  ...
  cairo_pdf('./plot/evolution_metrics_Ethnicity_By_nonLac_vs_Lac_4.pdf', width = 4.45, height = 4.18, onefile = T)
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = expression(pi),titleLab = 'Lactobacillaceae',addJitter = T,
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T, jitterSize = 0.1, 
             y.position = seq(0.013, 0.013+0.002*7, 0.002)-0.002,aspectRatio = 1.2/1,x.angle = 30,
  ...
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             ...
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(0.026, 0.026 + 0.004*9, 0.004)-0.01,aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = 'SNVs/kbp',titleLab = 'Lactobacillaceae',
             yval = 'SNV_per_kbp', if_log10 = F, hide.ns = T,jitterSize = 0.1, addJitter = T,
             y.position = seq(20, 20+3*2, 2),aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = 'SNVs/kbp',titleLab = 'Non-Lactobacillaceae',
             yval = 'SNV_per_kbp',  if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(50, 50+10*7, 10),aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = "1 - D'",titleLab = 'Lactobacillaceae',
             yval = 'D_rev', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(0.4, 0.4 + 5*0.04, 0.04)-0.08,aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = "1 - D'",titleLab = 'Non-Lactobacillaceae',
             yval = 'D_rev',if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(0.22, 0.22+0.027*5, 0.027),aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = "pN/pS",titleLab = 'Lactobacillaceae',
             yval = 'pnps',if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(2, 2+0.2*3, 0.2)-0.1,aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Ethnicity2', yLab = "pN/pS",titleLab = 'Non-Lactobacillaceae',
             yval = 'pnps', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(1.5, 1.5+0.25*6, 0.25),aspectRatio = 1.2/1,x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  dev.off()
}
##### time trend & compare 3rd trimester and post-parturm ####
table(refMetData2$trimester)
refMetData2$Trimester = refMetData2$trimester
cairo_pdf('./plot/evolution_metrics_Trimester_By_nonLac_vs_Lac.pdf', width = 4.43, height = 3.65, onefile = T)
if(T){
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Trimester', yLab = "pN/pS",titleLab = 'Non-Lactobacillaceae',
             yval = 'pnps', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = c(1.45, 1.6),
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Trimester', yLab = "pN/pS",titleLab = 'Lactobacillaceae',
             yval = 'pnps', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             # y.position = seq(1.5, 1.5+0.25*6, 0.25),
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Trimester', yLab = expression(pi),titleLab = 'Lactobacillaceae',addJitter = T,
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T, jitterSize = 0.1, 
             y.position = seq(0.012, 0.012+0.001*2, 0.001),
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Trimester', yLab = expression(pi),titleLab = 'Non-Lactobacillaceae',addJitter = T,
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T, jitterSize = 0.1, 
             y.position = seq(0.023, 0.023+0.002*2, 0.002),
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Lactobacillaceae',], 
             xval = 'Trimester', yLab = "1 - D'",titleLab = 'Lactobacillaceae',
             yval = 'D_rev', if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(0.3, 0.3 + 3*0.04, 0.04)+0.07,
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = refMetData2[refMetData2$if_Lactobacillaceae == 'Non-Lactobacillaceae',], 
             xval = 'Trimester', yLab = "1 - D'",titleLab = 'Non-Lactobacillaceae',
             yval = 'D_rev',if_log10 = F, hide.ns = T, jitterSize = 0.1, addJitter = T,
             y.position = seq(0.25, 0.25+0.03*4, 0.03),
             aspectRatio = 1.2/1,
             # x.angle = 30,
             expandMult = expansion(mult = c(0.03, 0.1)))
} 
...
if(T){
  ...
  pp = p %>% facet(nrow = 1, facet.by = 'Metrics', scales = 'free_x') +
    ...
    coord_flip()
  cairo_pdf('./plot/evolution_metrics_nonLac_vs_Lac.pdf', width = 9.78, height = 1.78)
  print(pp)
  dev.off()
}
...
if(T){
  ...
  if(plotFlag){
    openxlsx::write.xlsx(comp_DF, file = './data/microdiversity_pairwise_group_comp_2.xlsx')  
    openxlsx::write.xlsx(uniRes, file = './data/microdiversity_uni_regression_v2.xlsx')
  }
}
...
# merge
uniRes = readxl::read_xlsx('./data/microdiversity_uni_regression_v2.xlsx')
res = merge(uniRes, lmeRes, by = c('species', 'y', 'x'), all = T)
# 
# openxlsx::write.xlsx(res, './data/microdiveristy_lme_regression_fixed_v2.xlsx')
...
openxlsx::write.xlsx(res_mulisig, './data/microdiveristy_lme_regression_fixed_sig_v2.xlsx')

##### plot boxplot for candidate species and variable all references and mags #####
if(plotFlag){
  pdf('./plot/microdiveristy_boxplot_term.pdf', width = 10, height = 7)
}
ysNorm = list(nucl_diversity = 'Nucleotide diversity', dnds = 'dN/dS', pnps = 'pN/pS', dn_ds_common = 'dN/dS (common)')
x = 'Term_char2'
y = 'nucl_diversity' # 'nucl_diversity'
...
res_mulisig = read.xlsx('./data/microdiveristy_lme_regression_fixed_sig_v2.xlsx')
...
if(T){
  ...
  sp = 'Lactobacillus crispatus'
  tmpDF = refMetData[refMetData$genome_species == sp, ]
  relAbuPlot(data0 = tmpDF, xval = 'Ethnicity2', yLab = "1 - D'",titleLab = sp,
             yval = 'D_rev', if_log10 = F, hide.ns = T, 
...
dev.off()
##### carefully!!!! ####
cairo_pdf('./plot/microdiversity_super_good_results_exposome.pdf', width = 6.05,height = 3.55, onefile = T)
if(T){
  sp = 'Lactobacillus iners'
  tmpDF = refMetData[refMetData$genome_species == sp, ]
  relAbuPlot(data0 = tmpDF, xval = 'Marriage', yLab = 'pN/pS',titleLab = sp,
             yval = 'pnps',if_log10 = F, hide.ns = T, aspectRatio = 1,
             y.position = seq(1.5, 1.5+0.2*2, 0.2),
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = tmpDF, xval = 'Depression', yLab = 'pN/pS',titleLab = sp,
             yval = 'pnps',if_log10 = F, hide.ns = T, aspectRatio = 1.2,
             # y.position = seq(0.012, 0.012+0.001*1, 0.001),
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = tmpDF, xval = 'Employment', yLab = 'pN/pS',titleLab = sp,
             yval = 'pnps',if_log10 = F, hide.ns = T, aspectRatio = 1,
             # y.position = seq(0.012, 0.012+0.001*1, 0.001),
  ...
  relAbuPlot(data0 = tmpDF, xval = 'Marriage', yLab = expression(pi),titleLab = sp,
             yval = 'nucl_diversity',if_log10 = F, hide.ns = T, aspectRatio = 1,
             y.position = seq(0.012, 0.012+0.001*1, 0.001),
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = tmpDF, xval = 'Depression', yLab = expression(pi),titleLab = sp,
             yval = 'nucl_diversity',if_log10 = F, hide.ns = T, aspectRatio = 1.2,
             # y.position = seq(0.012, 0.012+0.001*1, 0.001),
             expandMult = expansion(mult = c(0.03, 0.1)))
  ...
  tmpDF = refMetData[refMetData$genome_species == sp, ]
  tmpDF$Trimester = tmpDF$trimester
  relAbuPlot(data0 = tmpDF, xval = 'Marriage', yLab = "1 - D'",titleLab = sp,
             yval = 'D_rev', if_log10 = F, hide.ns = T, 
             y.position =  seq(0.18, 0.18+0.02*1, 0.02),
             aspectRatio = 1,
             expandMult = expansion(mult = c(0.03, 0.1)))
  relAbuPlot(data0 = tmpDF, xval = 'Employment', yLab = expression(pi), titleLab = sp,
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T, 
             # y.position = c(0.021, 0.023),
             aspectRatio = 1, 
             expandMult = expansion(mult = c(0.03, 0.1)))
  ...
  sp = 'Lactobacillus crispatus'
  tmpDF = refMetData[refMetData$genome_species == sp, ]
  relAbuPlot(data0 = tmpDF, xval = 'FOB', yLab = expression(pi),titleLab = sp,
             yval = 'nucl_diversity', if_log10 = F, hide.ns = T,
             y.position = 0.0075,
             aspectRatio = 1.2,
             expandMult = expansion(mult = c(0.03, 0.1)))
...
relAbuPlot(data0 = tmpDF, xval = 'Term', yLab = expression(pi),titleLab = sp,
           ...
           aspectRatio = 1,
           expandMult = expansion(mult = c(0.03, 0.1)))
# colorTerm2 = c('Early-term', 'Ful')
tmpDF$Term2 = tmpDF$Term_char2
relAbuPlot(data0 = tmpDF, xval = 'Term2', yLab = expression(pi),titleLab = sp,
           yval = 'nucl_diversity', if_log10 = F, hide.ns = T, 
           # y.position =  seq(0.18, 0.18+0.02*1, 0.02),
           aspectRatio = 1,
           expandMult = expansion(mult = c(0.03, 0.1)))

tmpDF = refMetData[refMetData$if_Lactobacillaceae == 'Lactobacillaceae', ]
tmpDF$Trimester = tmpDF$trimester
tmpDF$Term = tmpDF$Term_char
relAbuPlot(data0 = tmpDF, xval = 'Term', yLab = expression(pi),titleLab = 'Lactobacillaceae',
           yval = 'nucl_diversity', if_log10 = F, hide.ns = T, 
           # y.position =  seq(0.18, 0.18+0.02*1, 0.02),
           aspectRatio = 1,
           expandMult = expansion(mult = c(0.03, 0.1)))
...
# res = readxl::read_xlsx('./data/microdiveristy_lme_regression_fixed_v2.xlsx')
res = merge(uniRes, lmeRes, by = c('species', 'y', 'x'), all = T)
dim(res)
table(res$y)
...
uniqueVar$sp_y = paste0(uniqueVar$species %>% str_sub(1,1), '.',
                        ...
                        ' (', uniqueVar$y, ')')
uniqueVar$value = uniqueVar$lme.multi.P_value_log
...
table(col_fun$col)
library(circlize)
cairo_pdf('./plot/circos_microdiveristy_v3.pdf', width = 6.25, height = 8.52)
...
cairo_pdf('./plot/lme_microdiversity_2.pdf', width = 4.08, height = 3.01, onefile = T)
if(T){
  ...
  for(sp in unique(refMetData$genome_species)){
    for(yv in ys){
      ...
      lmeDF = lmeDF[, c('y', 'Sample_GA',  'pt_ID', 'Term2', 'Ethnicity2')]
      nrow(lmeDF)
      try({
