...
load('./data/RData/refDF_minCovBreath10_re.RData')
...
if(F){
  ...
  if(T){
    ...
    cairo_pdf('./plot/SNVs_vs_D_vs_coverage_vs_pNpS_3_RE.pdf', width = 8, height = 3.63, onefile = T)
    dim(refMetData2)
    ...
    write.xlsx(refMetData2_paper, file = './table/evoluionary_metrices_this_study_RE.xlsx')
  ...
  if(T){
    cairo_pdf('./plot/pi_vs_pNpS_vs_coverage_vs_D_RE.pdf', width = 5.5, height = 5, onefile = T)
    ...
    pnpsDF = refMetData2 %>% group_by(genome_species) %>% 
      summarise(mean_pNpS = mean(pnps, na.rm = T),
    ...
    wx4 = ggplot(pnpsDF, aes(x = mean_pi, y = mean_pNpS, size = mean_D, fill = Family)) +
      ...
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/evo_type_RE.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx4)
    dev.off()
    ...
    wx5 = ggplot(pnpsDF, aes(x = mean_pi, y = mean_pNpS, size = mean_coverage, fill = Family)) +
      ...
      mytheme3 + theme(legend.spacing.y = unit(0.1, 'cm'), 
                       legend.box.spacing = unit(0.05, 'cm'), aspect.ratio = 1)
    cairo_pdf('./plot/evo_type3_RE.pdf', width = 6.84, height = 3.72, onefile = T)
    print(wx5)
    dev.off()
  }
...
if(T){
  cairo_pdf('./plot/evolution_metrics_nonLac_vs_Lac2_RE.pdf', width = 10.47, height = 1.78, onefile = T)
